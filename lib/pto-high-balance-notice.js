...
function buildPtoHighBalanceNotice({ ptoAvailableHours, ptoMaxCarryoverHours, employeeName }) {
  ...
  const summary = `You currently have ${formatHours(available)} hours of PTO available. At year-end, only ${formatHours(carryover)} hours can carry over to the next year.`;
  ...
  const text = [
    ...
    "",
    `At year-end, only ${formatHours(carryover)} hours of PTO can carry over to the next year.`,
    "",
...
<html lang="en"><body style="font-family:Inter,Segoe UI,sans-serif;line-height:1.5;color:#1c1917">
  ...
  <p>At year-end, only <strong>${formatHours(carryover)} hours</strong> of PTO can carry over to the next year.</p>
