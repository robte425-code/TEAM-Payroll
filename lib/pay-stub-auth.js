...
const { authEnabled, respondAuthMisconfigured } = require("./authConfig");
const { resolveIsAdmin } = require("./apiAuth");

async function getAuthContext(req, res) {
  if (respondAuthMisconfigured(res)) {
    return { error: { status: 503, message: "Authentication is not configured" } };
  }
  const secret = process.env.NEXTAUTH_SECRET;
  if (!secret) {
    return { error: { status: 503, message: "Auth not configured" } };
  }
  ...
  const isAdmin = await resolveIsAdmin(realEmail);
