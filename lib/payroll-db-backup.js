...
function sqlValue(value, dataType, udtName) {
  ...
  if (dataType === "ARRAY" || (typeof udtName === "string" && udtName.startsWith("_"))) {
    const arrayType =
      udtName === "_uuid" ? "uuid[]" : udtName === "_int4" ? "integer[]" : "text[]";
    if (Array.isArray(value)) {
      if (value.length === 0) return `'{}'::${arrayType}`;
      const literals = value.map((item) =>
        item === null || item === undefined ? "NULL" : sqlString(String(item))
      );
      return `ARRAY[${literals.join(", ")}]::${arrayType}`;
    }
    if (value === "" || value === "{}") return `'{}'::${arrayType}`;
    if (typeof value === "string" && value.startsWith("{") && value.endsWith("}")) {
      return `${sqlString(value)}::${arrayType}`;
    }
    return `'{}'::${arrayType}`;
  }
...
function validatePayrollBackupSql(sql) {
  if (!sql || typeof sql !== "string" || !sql.trim()) {
    throw new Error("Empty backup file.");
  }
  if (!sql.includes(BACKUP_HEADER)) {
    throw new Error(
      "Invalid backup file. Please upload a file downloaded from this Backup page."
    );
  }
}

async function restorePayrollBackupSql(pool, sql) {
  validatePayrollBackupSql(sql);
