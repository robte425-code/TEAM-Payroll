// The one copy, shared with the analyzer page. See its header for why it lives
// under public/ and why having two of it would fail silently.
const { buildCalcRowKey } = require("../public/shared/calc-row-key");

/**
 * A problem with what the caller sent, as opposed to a problem with us.
 *
 * Without the distinction every failure came back as 400, so a database outage
 * was reported to the operator as though they had typed something wrong, and
 * monitoring that separates 4xx from 5xx saw a client error during a real one.
 */
...
async function excludeRow(pool, { payrollEndDate, row, reason, excludedByEmail }) {
  ...
  if (!row || typeof row !== "object") throw new ValidationError("Billing line is required.");

  const rowKey = String(row.rowKey || buildCalcRowKey(row)).trim();
  if (!rowKey) throw new ValidationError("Billing line key is required.");
  ...
  if (!why) throw new ValidationError("A reason is required to hold a line out of pay.");
...
async function includeRow(pool, { payrollEndDate, rowKey }) {
  ...
  if (!key) throw new ValidationError("Billing line key is required.");
