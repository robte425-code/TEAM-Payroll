...
function resolveLniRatesForDate(rateRows, employmentType, asOfDate) {
  ...
  for (const row of rateRows || []) {
    if (normalizeEmploymentType(row.employment_type) !== type) continue;
    const effectiveDate = formatSqlDate(row.effective_date);
    if (!effectiveDate || effectiveDate > date) continue;
    if (!resolved || effectiveDate > formatSqlDate(resolved.effective_date)) {
      resolved = row;
  ...
  return {
    employmentType: type,
    effectiveDate: formatSqlDate(resolved.effective_date),
    professionalRate: toNumber(resolved.professional_rate),
    travelWaitRate: toNumber(resolved.travel_wait_rate),
  };
}
