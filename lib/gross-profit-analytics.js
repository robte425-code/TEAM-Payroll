...
function buildGrossProfitAnalytics({
  ...
  for (const row of payrollRows || []) {
    ...
    if (!employeeId) {
      warnings.add(`No employee record match for ${employeeName}; using Full VRC default and rates.`);
    }

    const rates = resolveLniRatesForDate(lniRateSchedules, employmentType, payrollStartDate);
    if (!rates) {
      warnings.add(
        `No L&I billing rates for ${employmentType === "intern" ? "Intern" : "Full VRC"} on or before ${payrollStartDate}.`
      );
    }
